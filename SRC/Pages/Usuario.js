...
export default function Usuario({ navigation }) {
  ...
  const LoginUsuario = async () => {
    try {
      ...
      navigation.navigate("Loja", { idUser: userCredential.user.uid }); //task de cada usuario
    } catch (error) {
  ...
  useEffect(() => {
    const statusAuth = onAuthStateChanged(auth, (user) => {
      if (user) {
        navigation.navigate("Loja", { idUser: user.uid });
      }
    });
  ...
  return (
    <SafeAreaView style={styles.contaner}>
      ...
      {email === "" || password == "" ? (
        ...
        <TouchableOpacity onPress={LoginUsuario}  style={styles.botao}>
          <Text style={styles.botaotxt}>Login</Text>
        </TouchableOpacity>
      )}
